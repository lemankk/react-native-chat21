...
export class ChatConversationHandler {
  ...
  public messages: any[];
  public messagesRef: FirebaseDatabaseTypes.Query;
  public messagesRefTyping: FirebaseDatabaseTypes.Query;
  ...
  setStatusMessage(item, conversationWith) {
    ...
    if (item.val()["status"] < MSG_STATUS_RECEIVED) {
      ...
      const msg = item.val();
      if (
        msg.sender != this.loggedUser.uid &&
        msg.status < MSG_STATUS_RECEIVED
      ) {
        getDbRef(
          `users/${this.loggedUser.uid}/messages/${conversationWith}/${item.key}`
        ).update({ status: MSG_STATUS_RECEIVED });
      }
    }
  }
  /**
  ...
  sendMessage(
    ...
    metadata,
    conversationWith,
    conversationWithDetailFullname,
    channel_type
  ) {
  ...
  getWritingMessages() {
    ...
    this.messagesRefTyping.on("child_changed", (childSnapshot) => {
      ...
      events.emit(
        "isTypings-" + this.conversationWith,
        childSnapshot,
   ...
   *
   */
  setWritingMessages(str: string, channel_type?: string) {
    ...
    this.setTimeoutWritingMessages = setTimeout(() => {
      ...
      if (channel_type === TYPE_GROUP) {
        console.log("GRUPPO");
        readUrlNodeTypings = getDbPath(
          `${readUrlNodeTypings}/${this.loggedUser.uid}`
        );
      }
