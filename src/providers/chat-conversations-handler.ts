...
export class ChatConversationsHandler {
  ...
  connect() {
    ...
    const { app } = config;
    const urlNodeFirebase = conversationsPathForUserId(
      this.userId
    );
  ...
  added(childSnapshot) {
    ...
    if (this.isValidConversation(childSnapshot.key, conversation)) {
      ...
      this.tiledeskConversationsProvider.setClosingConversation(
        childSnapshot.key,
        false
      );
      const index = searchIndexInArrayForUid(
        this.conversations,
        conversation.uid
      );
      if (index > -1) {
      ...
      console.error(
        "ChatConversationsHandler::added::conversations with conversationId: ",
        childSnapshot.key,
        "is not valid"
      );
    }
  ...
  changed(childSnapshot) {
    ...
    if (this.isValidConversation(childSnapshot.key, conversation)) {
      ...
      console.error(
        "ChatConversationsHandler::changed::conversations with conversationId: ",
        childSnapshot.key,
        "is not valid"
      );
    }
  ...
  removed(childSnapshot) {
    ...
    const { app } = config;
    console.log(
      "ChatConversationsHandler::onSnapshotRemoved::conversation:",
      childSnapshot.key
    );
    const index = searchIndexInArrayForUid(
      this.conversations,
    ...
    this.tiledeskConversationsProvider.deleteClosingConversation(
      childSnapshot.key
    );
  ...
  addConversationListener(uidUser, conversationId) {
    ...
    console.log(
      "ChatConversationsHandler::addConversationListener::reference:",
      url,
      reference.toString()
    );
