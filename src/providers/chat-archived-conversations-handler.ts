...
export class ChatArchivedConversationsHandler {
  ...
  private onSnapshotAdded(snapshot) {
    const childData: IConversation = snapshot.val();
    ...
    if (this.isValidConversation(snapshot.key, conversation)) {
      ...
      console.error(
        "ChatArchivedConversationsHandler::added::conversations with conversationId: ",
        snapshot.key,
        "is not valid"
      );
    }
  ...
  private onSnapshotChanged(snapshot) {
    ...
    if (this.isValidConversation(snapshot.key, conversation)) {
      ...
      console.error(
        "ChatArchivedConversationsHandler::changed::conversations with conversationId: ",
        snapshot.key,
        "is not valid"
      );
    }
  ...
  private onSnapshotRemoved(childSnapshot) {
    ...
    console.log(
      "ChatArchivedConversationsHandler::onSnapshotRemoved::conversation:",
      childSnapshot.key
    );
    const index = searchIndexInArrayForUid(
      this.conversations,
   ...
   * @param conv
   */
  private completeConversation(conv): IConversation {
    // debugger;
    conv.selected = false;
    if (
