...
export class ChatPresenceHandler {
    ...
    lastOnlineForUser(userId) {
        ...
        const { app } = config;
        const lastOnlineRefURL = this.urlNodeFirebase + lastOnlineDbRef(userId);
        ...
        lastOnlineRef.on("value", (child) => {
            if (child.val()) {
                ...
                events.emit("lastConnectionDate-" + userId, userId, lastConnectionDate);
            }
            else {
            }
        });
     ...
     * @param timestamp
     */
    getTimeLastConnection(timestamp) {
...
//# sourceMappingURL=chat-presence-handler.js.map