...
export class UserService {
    constructor(instance, chatPresenceHandler, msgService) {
        this.instance = instance;
        this.chatPresenceHandler = chatPresenceHandler;
        this.msgService = msgService;
        this.callbackGetToken = (token) => {
    ...
    //   return userFirebase;
    // }
    saveCurrentUserDetail(uid, email, firstname, lastname) {
            ...
            if (member.trim() !== "" && member.trim() !== SYSTEM) {
                this.getUserDetail(member)
                    ...
                    if (snapshot.val()) {
                        ...
                        userDetail = new UserModel(snapshot.key, user.email, user.firstname, user.lastname, fullname.trim(), imageUrl);
            ...
            if (!user) {
                ...
                userFirebase.on("value", (snapshot) => {
                    if (snapshot.val()) {
                        const user = snapshot.val();
                        const fullname = user.firstname + " " + user.lastname;
                        this.currentUserDetails = new UserModel(user.uid, user.email, user.firstname, user.lastname, fullname, user.imageurl);
                    }
                    else {
                        this.currentUserDetails = new UserModel(user.uid, user.email, "", "", user.uid, "");
